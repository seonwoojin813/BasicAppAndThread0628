...
public class MainActivity extends AppCompatActivity {
    ...
    Button contactbtn, camerabtn, phonebtn, browerbtn, mapbtn;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        browerbtn = (Button) findViewById(R.id.browserbtn);
        ...
        contactbtn.setOnClickListener((view) -> {
            ...
            //다음 액티비티를 화면에 출력하고 데이터를 돌려받을수는 없습니다.
        ...
        mapbtn.setOnClickListener((view) -> {
            Intent intent = new Intent(Intent.ACTION_VIEW,
                    Uri.parse("geo:37.5662, 126.9779"));
            startActivity(intent);
        });

        //크롬 앱을 호출
        browerbtn.setOnClickListener((view) -> {
            Intent intent = new Intent(Intent.ACTION_VIEW,
                    Uri.parse("http://www.daum.net"));
            startActivity(intent);
        });

        //전화앱을 호출 - 실시간으로 권한을 확인
    ...
    public void onActivityResult(
            int requestCode, int responseCode, Intent intent) {
        if (requestCode == 100) {
            //선택한 전화번호의 문자열을 출력
            //마지막 부분이 id입니다.
    ...
    public void onSaveInstanceState(Bundle bundle) {
        ...
        //안드로이드에서는 직렬화하는 인터페이스 2개
        //자바에서 사용하는 Serializable
        //안드로이드에서 추가된 Parselable
        if (bitmap != null) {
